		...
		foreach ( $sidebars_widgets as $widget_area => $widgets ) {
			...
			foreach ( $widgets as $widget_i => $widget_id ) {
				...
				if ( $wc->check_widget_visibility( $widget_id ) ) {
					$status = sprintf( __( 'Showing <strong>%1$s</strong> in "%2$s"' ), esc_html( $widget_id ), esc_html( $widget_area ) );
				} else {
					$status = sprintf( __( 'Hiding <strong>%1$s</strong> in "%2$s"' ), esc_html( $widget_id ), esc_html( $widget_area ) );
				}
